...
function loadScript( evt, filePath, stat){
  ...
  if( path.extname(filePath) =='.js' && !skipDirs[versionDir] ){
    ...
    //unloadScript
    if(evt == 'unlink'){ unloadScript(filePath); }
    else{

      //unload if existing...
      if( apiFiles[filePath] ){ unloadScript(filePath); }

      //log...
      logLined( 'blue', util.format("LOADING ROUTE FILE: %s - [%d reloads]", path.join( thisDirectory , path.basename(filePath) ) , (apiReloads[filePath] || 0) ) );
      ...
      _.each(routeFile.routes, (obj, index) => {
        ...
        if( ( name = apiServer[obj.method] ( _.pick(obj,'path','name') , obj.middleware.before, obj.func , obj.middleware.after ) ) ){
          //update name...
          routeFile.routes[index].name = name;

          //log
          log( chalk.magenta( util.format("Route %s [%s] - (%s) mounted...", chalk.bold( obj.path ), chalk.bold(obj.method.toUpperCase()), routeFile.routes[index].name  )));
