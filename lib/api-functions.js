...
function initialize(routeFile) {
  ...
  if (!diff.length) {
    ...
    var error = new Error(
      util.format(
        "All required options not entered. Please complete the following options: %s",
        util.inspect(diff)
      )
    );
  ...
  bunyanLogger = bunyan.createLogger({
    ...
    streams: [
      ...
      {
        path: path.join(
          this.options.apiLogDir,
          this.options.apiName.replace(/[^a-z0-9]/gi, "_") + ".log"
        ),
        level: "trace"
      }
    ],
  ...
  if (!this.options.apiServerOpts.name) {
    this.options.apiServerOpts.name = this.options.apiName;
  }
...
function start(pluginsArr) {
  ...
  apiServer.listen(apiOptions.apiPort, function() {
    logLined(
      "green",
      util.format(
        "%s: NOW LISTENING AT %s",
        chalk.bold(process.title),
        chalk.bold(apiServer.url)
      )
    );
    ...
    _.each(_.compact(arrify(pluginsArr)), function(plugin) {
      log(
        chalk.magenta(
          util.format("Plugin: %s set.", chalk.bold(plugin.name || "~ Unknown"))
        )
      );
...
function makeRoutes() {
  ...
  var watcher = chokidar
    .watch(watchDirs, chokidarOpts)
    .on("all", function(evt, filePath, stat) {
      //
      if (
        (stat && !stat.isDirectory()) ||
        (path.extname(filePath) == ".js" || path.basename(filePath) == ".skip")
      ) {
...
function unloadScripts(dirs) {
  ...
  _.each(dirs, dir => {
    ...
    _.each(glob.sync(path.join(dir, "*.js")), filePath =>
      unloadScript(path.resolve(filePath))
    );
  });
}
...
function unloadScript(filePath) {
  if (apiFiles[filePath]) {
    ...
    logLined(
      "white",
      util.format(
        "UNLOADING ROUTE FILE: %s ",
        path.join(thisDirectory, path.basename(filePath))
      )
    );
...
function unmountRoute(name, filePath) {
  ...
  if ((unmounted = apiServer.rm(name))) {
    ...
    if (i > -1) {
      ...
      apiFiles[filePath].routes = _.compact(apiFiles[filePath].routes);
    }
  }
}
  ...
  if (path.extname(filePath) == ".js" && !skipDirs[versionDir]) {
    ...
    if (evt == "unlink") {
      ...
      logLined(
        "blue",
        util.format(
          "LOADING ROUTE FILE: %s - [%d reloads]",
          path.join(thisDirectory, path.basename(filePath)),
          apiReloads[filePath] || 0
        )
      );
      ...
      _.each(routeFile.routes, (obj, index) => {
        ...
        if (
          (name = apiServer[obj.method](
            _.pick(obj, "path", "name"),
            obj.middleware.before,
            obj.func,
            obj.middleware.after
          ))
        ) {
          //update name...
          ...
          log(
            chalk.magenta(
              util.format(
                "Route %s [%s] - (%s) mounted...",
                chalk.bold(obj.path),
                chalk.bold(obj.method.toUpperCase()),
                routeFile.routes[index].name
              )
            )
          );
        } else {
          //remove route index if we cannot mount it...
      ...
      apiReloads[filePath] = (apiReloads[filePath] || 0) + 1;
...
function routeFileData(filePath, cb) {
  ...
  if (_.has(routeFile, "$GLOBALS$")) {
    ...
    MIDDLEWARE.before = _.union(
      MIDDLEWARE.before,
      arrify(_.values(GLOBALS.middleware.before))
    );
    MIDDLEWARE.after = _.union(
      MIDDLEWARE.after,
      arrify(_.values(GLOBALS.middleware.after))
    );
  }
  ...
  _.each(routeFile, function(routeData, route) {
    if (_.has(routeData, "middleware")) {
      MIDDLEWARE.before = _.union(
        MIDDLEWARE.before,
        arrify(_.values(routeData.middleware.before))
      );
      MIDDLEWARE.after = _.union(
        MIDDLEWARE.after,
        arrify(_.values(routeData.middleware.after))
      );
    }
    ...
    _.each(routeData.methods, function(func, method) {
      ...
      if (/^\^/.test(route)) {
        routeData.path = util
          .format("/%s", route.replace(/^\^/, ""))
          .replace(/\/{2,}/, "/");
      } else {
        routeData.path = util
          .format("/%s/%s", version, route)
          .replace(/\/{2,}/, "/");
      }
